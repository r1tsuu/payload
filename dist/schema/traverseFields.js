...
import { PgNumericBuilder, PgUUIDBuilder, PgVarcharBuilder, boolean, foreignKey, index, integer, jsonb, pgEnum, text, timestamp, varchar } from 'drizzle-orm/pg-core';
...
export const traverseFields = ({ adapter, columnPrefix, columns, disableNotNull, disableUnique = false, fieldPrefix, fields, forceLocalized, indexes, localesColumns, localesIndexes, newTableName, parentTableName, relationsToBuild, relationships, rootRelationsToBuild, rootTableIDColType, rootTableName, versions })=>{
    ...
    fields.forEach((field)=>{
        ...
        if (fieldAffectsData(field)) {
            ...
            if (adapter.payload.config.localization && (field.localized || forceLocalized) && field.type !== 'array' && field.type !== 'blocks' && ('hasMany' in field && field.hasMany !== true || !('hasMany' in field)) && ('dbJsonColumn' in field && field.dbJsonColumn !== true || !('dbJsonColumn' in field))) {
            ...
            case 'number':
                {
                    if (field.hasMany) {
                        ...
                        targetTable[fieldName] = withDefault(numberColumnMap[field.dbType ?? 'numeric'](fieldName), field);
                    }
                    break;
                }
            case 'richText':
